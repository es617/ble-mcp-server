"""Tests for ble_mcp_server.trace — no BLE hardware required."""
...
class TestSanitizeArgs:
    def test_strips_payloads_when_disabled(self, monkeypatch):
        monkeypatch.setattr("ble_mcp_server.trace.TRACE_PAYLOADS", False)
        args = {"connection_id": "c1", "char_uuid": "2a00", "value_b64": "AQID", "value_hex": "010203"}
        result = sanitize_args(args)
        assert "value_b64" not in result
        assert "value_hex" not in result
        assert result["connection_id"] == "c1"
        assert result["char_uuid"] == "2a00"

    def test_passes_through_non_payload_args(self, monkeypatch):
        monkeypatch.setattr("ble_mcp_server.trace.TRACE_PAYLOADS", False)
        args = {"connection_id": "c1", "char_uuid": "2a00"}
        result = sanitize_args(args)
        assert result == args

    def test_truncates_when_payloads_enabled(self, monkeypatch):
        monkeypatch.setattr("ble_mcp_server.trace.TRACE_PAYLOADS", True)
        monkeypatch.setattr("ble_mcp_server.trace.TRACE_MAX_BYTES", 10)
        args = {"value_b64": "A" * 20}
        result = sanitize_args(args)
        assert result["value_b64"] == "A" * 10 + "\u2026"

    def test_no_truncation_when_under_limit(self, monkeypatch):
        monkeypatch.setattr("ble_mcp_server.trace.TRACE_PAYLOADS", True)
        monkeypatch.setattr("ble_mcp_server.trace.TRACE_MAX_BYTES", 100)
        args = {"value_hex": "0102"}
        result = sanitize_args(args)
        assert result["value_hex"] == "0102"
...
class TestInitTrace:
    def test_returns_none_when_disabled(self, monkeypatch):
        monkeypatch.setattr("ble_mcp_server.trace.TRACE_ENABLED", False)
        monkeypatch.setattr("ble_mcp_server.trace._buffer", None)
        from ble_mcp_server.trace import init_trace
...
class TestHandlersTraceDisabled:
    ...
    async def test_status_disabled(self, monkeypatch):
        monkeypatch.setattr("ble_mcp_server.trace._buffer", None)
        from ble_mcp_server.handlers_trace import handle_trace_status
    ...
    async def test_tail_disabled(self, monkeypatch):
        monkeypatch.setattr("ble_mcp_server.trace._buffer", None)
        from ble_mcp_server.handlers_trace import handle_trace_tail
...
class TestHandlersTraceEnabled:
    ...
    async def test_status_enabled(self, monkeypatch):
        buf = TraceBuffer()
        buf.emit({"event": "test"})
        monkeypatch.setattr("ble_mcp_server.trace._buffer", buf)
        from ble_mcp_server.handlers_trace import handle_trace_status
